    ...
    def print_Running(self, int_Progress = 0, str_AdditionalText = '', pre_Text = '', ind_Simple = False):
        ...
        if ind_Simple == False:
            if str_AdditionalText == '':
               str_Text = pre_Text + '[' + self.last_Char + '] ' + str(int_Progress) + ' [Elapsed Time: ' + ':'.join(str(str(timedelta(seconds=dht_Now - self.dht_Inicio))).split('.')[:1]) + '] '   
            else:
               str_Text = pre_Text + '[' + self.last_Char + '] ' + str(int_Progress) + ' ' + str_AdditionalText + ' [Elapsed Time: ' + ':'.join(str(str(timedelta(seconds=dht_Now - self.dht_Inicio))).split('.')[:1]) + '] '   
        else:
            if str_AdditionalText == '':
        ...
        if int(max_Columns) - self.fixed_width >= 0:
            if int_Step * int_Progress > (int(max_Columns) - self.const_infSize) or int_Progress >= 100:
               ...
               if self.fixed_width > 0:
                    ...
                    str_Text = pre_Text + ' [Elapsed Time: ' + ':'.join(str(str(timedelta(seconds=dht_Now - self.dht_Inicio))).split('.')[:1]) + '] [' + '#' * int(int_StepProgress) + ' ' * (int(self.fixed_width - self.const_infSize) - int(int_StepProgress)) + '] (' + str(int_Progress) + '%)'   
               else:
                    int_StepProgress = int(max_Columns) - self.const_infSize - self.fixed_width
                    str_Text = pre_Text + ' [Elapsed Time: ' + ':'.join(str(str(timedelta(seconds=dht_Now - self.dht_Inicio))).split('.')[:1]) + '] [' + '#' * int(int_StepProgress) + ' ' * int(round((int(max_Columns) - self.fixed_width - self.const_infSize) - int(int_StepProgress))) + '] (' + str(int_Progress) + '%)'   
            else:
               if self.fixed_width > 0:
            ...
            str_Text = pre_Text + ' [Elapsed Time: ' + ':'.join(str(str(timedelta(seconds=dht_Now - self.dht_Inicio))).split('.')[:1]) + '] (' + str(int_Progress) + '%)'
            str_Text = str_Text + ('\r' if self.ind_NewLine == False else '\n')
            if self.pos_Line > 0:
                sys.stdout.write("\x1b7\x1b[%d;%df%s\x1b8" % (self.pos_Line, self.pos_Column, str_Text))
            else:
